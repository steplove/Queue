...
function ManualQueue() {
  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      ...
      const filteredPosts = data.filter(
        (post) => post.PresStatus ===  "Sent_to_doctor" && post.Station === 3
      );
  ...
  const handleSelectRoom = (room) => {
    ...
    let checkroom = checkrooms.filter(
      (post) => post.PresStatus ===  "Sent_to_doctor" && post.Rooms === rooms && post.Station === 3
    );
  ...
  const displayedPosts = posts.filter(
    (item) => item.PresStatus === "Registered" && item.Station === 3
  );
              ...
              {displayedPosts.length === 0 ? (
                ...
                <div style={boxContainer}>
                  {displayedPosts
                   .concat(placeholders)
                   .slice(0, 9)
                    .filter((item) => item.PresStatus === "Registered" && item.Station === 3)
                    .map((item, index) => (
                      <div
                        key={index}
                        style={boxStyle}
